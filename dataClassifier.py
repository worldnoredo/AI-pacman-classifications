...
def enhancedFeatureExtractorDigit(datum):
    ...
    for x in range(DIGIT_DATUM_WIDTH):
        for y in range(DIGIT_DATUM_HEIGHT):
            position = (x, y)
            # IF PIXEL IS WHITE
            if datum.getPixel(x, y) == 0 and position not in explored:
                count += 1
                explored.append(position)
                fringe.push(position)

                # Perform BFS to find whitespace region.
                while not fringe.isEmpty():
                    ...
                    # ADD ALL FOUR NEIGHBORS TO BE PROCESSED
                    for x_val in [-1, 1]:
                        next_x_pos = x_pos + x_val
                        if (next_x_pos >= 0 and next_x_pos < DIGIT_DATUM_WIDTH):
                            posList.append((next_x_pos, y_pos))

                    for y_val in [-1, 1]:
                        next_y_pos = y_pos + y_val
                        if (next_y_pos >= 0 and next_y_pos < DIGIT_DATUM_HEIGHT):
                            posList.append((x_pos, next_y_pos))

                    for dum in posList:
                        if datum.getPixel(dum[0], dum[1]) == 0 and dum not in explored:
                            # WHITE PIXEL CONNECTED TO ORIGINAL WHITE PIXEL: DO NOT COUNT AS ADDITIONAL REGION
                            explored.append(dum)
                            fringe.push(dum)

    # Add feature depending on how many regions of whitespace.
    for x in range(1,7):
        features[x] = 0
        if count == x:
            features[x] = 1
